package com.itmowei.motionassistant.util;
...
public class XiaomiApi {
    ...
    public static Map<String, Object> mainHandler(String user, String password, String steps) {
        Map<String, Object> retMap = new HashMap();
        try {
            String accessCode = getAccessCode(user, password, retMap);
            ...
            String time = getTime();
            //随机 10000<=stepNum<35001 的数 不包括35001
            int stepNum = 0;
            if ("".equals(steps) || steps == null) {
                stepNum = ThreadLocalRandom.current().nextInt(10000, 35001);
            } else {
                stepNum = Integer.parseInt(steps);
            }
            updateStep(appToken, user_id, time, stepNum, user);
            retMap.put("flag", true);
            retMap.put("msg", stepNum);
            return retMap;
        } catch (Exception e) {
    ...
    public static Map<String, Object> mainHandler(String user, String password) {
        Map<String, Object> retMap = new HashMap();
        try {
            String accessCode = getAccessCode(user, password, retMap);
            Map<String, String> login = login(accessCode);
            ...
            String time = getTime();
            //随机 10000<=stepNum<35001 的数 不包括35001
            int stepNum = ThreadLocalRandom.current().nextInt(10000, 35001);
            updateStep(appToken, user_id, time, stepNum, user);
            retMap.put("flag", true);
            retMap.put("msg", stepNum);
            return retMap;
